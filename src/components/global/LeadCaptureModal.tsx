...
export const LeadCaptureModal: React.FC = () => {
  ...
  // Helper to clear all scheduled triggers
  const clearAllTimers = () => {
    if (initialTimerRef.current) clearTimeout(initialTimerRef.current);
    if (recurringTimerRef.current) clearTimeout(recurringTimerRef.current);
  };

  // Schedule the 30-second recurring trigger
  const scheduleRecurringTrigger = () => {
    if (typeof window === "undefined") return;
    const isCaptured = localStorage.getItem("nexora_lead_captured") === "true";
    if (isCaptured) return;

    clearAllTimers();
    recurringTimerRef.current = setTimeout(() => {
      const stillNotCaptured = localStorage.getItem("nexora_lead_captured") !== "true";
      if (stillNotCaptured) {
        setIsOpen(true);
      }
    }, 30000);
  ...
  // Initial 10-second trigger on mount
  useEffect(() => {
    if (typeof window === "undefined") return;
    const isCaptured = localStorage.getItem("nexora_lead_captured") === "true";
    if (isCaptured) return;
  ...
  const handleDismiss = () => {
    setIsOpen(false);
    // User dismissed: re-trigger after 30 seconds
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Permanently mark lead as captured in localStorage
      if (typeof window !== "undefined") {
        localStorage.setItem("nexora_lead_captured", "true");
      }
      clearAllTimers();
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 select-none">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg bg-white dark:bg-[#00144A] rounded-3xl p-6 sm:p-9 shadow-2xl border border-slate-200 dark:border-[#002277] z-10 text-[#00144A] dark:text-white overflow-hidden"
          ...
          >
            ...
            <button
              onClick={handleDismiss}
              type="button"
              className="absolute top-4 right-4 p-2 rounded-full bg-slate-100 dark:bg-[#000B2B] text-slate-500 dark:text-slate-300 hover:text-[#00144A] dark:hover:text-white transition-colors cursor-pointer"
            ...
            {isSubmitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-slate-100 dark:bg-[#000B2B] text-[#0099BE] border border-slate-200 dark:border-[#002277] mb-3 shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-[#000B2B] border border-slate-200 dark:border-[#002277] text-xs sm:text-sm text-[#00144A] dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#00D2FF]"
                    />
                  </div>
